function check_api_key(apikey) {
    ...
    try {
        if(!apikey) return {
            error: 1,
            msg: 'missing api key. Please Contact Owner : https://www.facebook.com/www.xnxx.com169'
        }
        const data_apikey = require(global.APIKEY);
        if (!data_apikey.find(i => i.apikey == apikey)) {
            return {
                error: 1,
                msg: 'APIKEY is not correct! Please Contact Owner : https://www.facebook.com/Mohammad.Rakib240'
            }
        } else {
            ...
            if (APIKEY.request == 0) {
                return {
                    error: 1,
                    msg: 'Your APIKEY request has expired. Please Contact Owner : https://www.facebook.com/www.xnxx.com169'
                }
            } 
            else {
                if (APIKEY.type == 'free') {
